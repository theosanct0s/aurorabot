...
const ACTION = 'punch';
const EMOJI = '🥊';
const SEND_TITLE = 'Incoming punch!';
const RETURN_TITLE = 'Punch returned!';
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName(ACTION)
    .setDescription('Send a playful punch to someone.')
    .addUserOption((option) =>
      option
        .setName('target')
        .setDescription('Who should receive the punch?')
        .setRequired(true),
    ),
  async execute(interaction) {
    ...
    if (!targetUser) {
      await interaction.reply({ content: 'Pick someone to punch first.', ephemeral: true });
      return;
    }
    ...
    try {
      ...
      await interaction.reply({
        content: 'I could not fetch a punch right now. Try again later.',
        ephemeral: true,
      });
      return;
    }
    ...
    const description = `${interaction.user} throws a punch at ${targetUser}!`;
    ...
    collector.on('collect', async (buttonInteraction) => {
      ...
      if (buttonInteraction.user.id !== targetUser.id) {
        await buttonInteraction.reply({
          content: "Hey, you're trying to steal someone else's punch?",
          ephemeral: true,
        });
        return;
      }
      ...
      try {
        ...
        await buttonInteraction.reply({
          content: 'I could not send a return punch right now. Try again in a moment.',
          ephemeral: true,
        });
        return;
      }
      ...
      const returnEmbed = createActionEmbed(
        RETURN_TITLE,
        `${targetUser} sends a punch back to ${interaction.user}!`,
        returnGif,
      );
      ...
      collector.stop('punch-returned');
    });

    collector.on('end', async (_, reason) => {
      if (reason === 'messageDelete' || reason === 'punch-returned') return;
